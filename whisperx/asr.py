    ...
    def transcribe(
        ...
        if self.tokenizer is None:
            if vad_segments:
                ...
                start_sample = int(first_segment["start"] * SAMPLE_RATE)
                segment_audio = audio[start_sample:]
            else:
                # Fall back to the first 30 seconds of audio if no VAD segments found
                segment_audio = audio
            language = language or self.detect_language(segment_audio)
